...
namespace AnujShroff.CDCMigrationGuard;

internal static class UpdateChecker
{
    private const string PackageId = "anujshroff.cdcmigrationguard";
    private const string PackageDisplayId = "AnujShroff.CDCMigrationGuard";
    ...
    {
        HttpClient.DefaultRequestHeaders.UserAgent.ParseAdd("CDCMigrationGuard-UpdateCheck");
    }
    ...
    public static async Task PrintBannerIfAvailable(Task<(Version Current, Version Latest)?> checkTask)
    {
        try
        {
            ...
            AnsiConsole.MarkupLine($"[yellow]A newer version is available:[/] {result.Value.Latest} (you have {result.Value.Current})");
            AnsiConsole.WriteLine($"Update with: dotnet tool update --global {PackageDisplayId}");
        }
        catch
        {
        }
